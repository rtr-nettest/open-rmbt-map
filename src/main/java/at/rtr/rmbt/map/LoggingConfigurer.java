 ...
 * <ol>
 *   <li>{@code LOGGING_CONFIG_FILE_MAP} points at an existing file &rarr; load that logback config
...
public class LoggingConfigurer implements ApplicationListener<ApplicationReadyEvent> {
    ...
    private static final String APP_NAME = "map-service";
    ...
    public void onApplicationEvent(ApplicationReadyEvent event) {
        try {
            ...
            final String configFile = resolve(servletContext, env, "LOGGING_CONFIG_FILE_MAP");
            final String logHost = resolve(servletContext, env, "LOG_HOST");
            LOG.info("Logging: resolved LOGGING_CONFIG_FILE_MAP={}, LOG_HOST={}", configFile, logHost);
            ...
            if (configFile != null) {
                ...
                LOG.warn("Logging: LOGGING_CONFIG_FILE_MAP [{}] is not a readable file - ignoring it", configFile);
            }

            if (logHost == null) {
                LOG.info("Logging: no LOGGING_CONFIG_FILE_MAP and no LOG_HOST - console only");
                return;
            }
